...
if (!is_admin() && empty($_SESSION['impersonated_by_admin'])) {
    set_flash('error', "Access Denied: Only Administrator can edit staff accounts.");
...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    // Check unique username constraint (except current staff)
    if ($username && $username !== $staff['username']) {
        $stmt_check = $pdo->prepare("SELECT id FROM staff WHERE username = ? AND id != ?");
        $stmt_check->execute([$username, $id]);
        if ($stmt_check->fetch()) {
            $errors[] = "Username '{$username}' is already taken by another staff member.";
        }

        $stmt_admin = $pdo->prepare("SELECT id FROM admins WHERE username = ?");
    ...
    if (empty($errors)) {
        ...
        if ($password !== '') {
            ...
            $stmt_update = $pdo->prepare("
                UPDATE staff 
                SET name = ?, username = ?, password = ?, permissions = ?, status = ?
                WHERE id = ?
            ");
            $stmt_update->execute([$name, $username, $hashed_password, $json_perms, $status, $id]);
        } else {
            $stmt_update = $pdo->prepare("
                UPDATE staff 
                SET name = ?, username = ?, permissions = ?, status = ?
                WHERE id = ?
            ");
            $stmt_update->execute([$name, $username, $json_perms, $status, $id]);
        }

        set_flash('success', "Staff account '{$name}' updated successfully!");
        header("Location: list.php");
        exit;
    }
} else {
    $_POST['name'] = $staff['name'];
    $_POST['username'] = $staff['username'];
    $_POST['status'] = $staff['status'];
    $_POST['permissions'] = json_decode($staff['permissions'] ?? '[]', true);
}
    ...
    <header class="bg-white border-b border-slate-200 px-8 py-4 flex justify-between items-center">
        <div>
            <h1 class="text-xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
                <i class="fas fa-user-edit text-blue-600"></i> Edit Staff Account & Permissions
            </h1>
            <p class="text-xs text-slate-500 mt-0.5">Modify staff profile, status, and feature permission checkboxes</p>
        </div>
    ...
    <main class="p-8">
        ...
        <form method="POST" class="bg-white rounded-xl shadow-sm border border-slate-200 p-8 max-w-3xl space-y-8">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                    ...
                    <div>
                        ...
                        <input type="text" name="username" value="<?= htmlspecialchars($_POST['username'] ?? '') ?>" required class="w-full px-3.5 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none font-mono">
                    </div>

                    <div>
                        <label class="block font-medium text-slate-700 mb-1">New Password (Leave blank to keep existing)</label>
                        <input type="password" name="password" placeholder="Enter new password (optional)" class="w-full px-3.5 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    </div>
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 mb-6">Toggle feature checkboxes to update module access for this staff member:</p>

                <div class="space-y-3">
                    <?php 
                    $selected_perms = $_POST['permissions'] ?? [];
                    if (!is_array($selected_perms)) $selected_perms = [];
